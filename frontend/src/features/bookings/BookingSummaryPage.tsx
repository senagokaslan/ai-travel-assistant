...
type FlightSummary = {
  kind: 'flight'; title: string; confirmationRequired: boolean; priceChanged: boolean; quotedTotal?: number; totalPrice: number; currency: string; searchUrl: string
...
function requestFromParams(params: URLSearchParams) {
  ...
  if (kind === 'hotel') return {
    ...
    body: {
      ...
      childAges: (params.get('childAges') ?? '').split(',').filter(Boolean).map(Number), quotedTotal: Number(params.get('quotedTotal')) || null,
    },
  }
  if (kind === 'flight') return {
    ...
    body: {
      ...
      adults: Number(params.get('adults')), children: Number(params.get('children')), infants: Number(params.get('infants')), quotedTotal: Number(params.get('quotedTotal')) || null,
    },
  }
  return null
}
...
export function BookingSummaryPage() {
  ...
  useEffect(() => {
    ...
    fetch(request.endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request.body), signal: controller.signal })
      .then(async response => {
        ...
        if (response.status === 409) { setError(payload.message ?? 'Seçilen ürün artık kullanılamıyor.'); setState('gone'); return null }
        if (!response.ok) throw new Error(payload.message ?? 'Rezervasyon özeti oluşturulamadı.')
        return payload as BookingSummary
      })
      .then(data => { if (data) { setSummary(data); setState('ready') } })
      .catch(fetchError => { if (fetchError.name !== 'AbortError') { setError(fetchError instanceof Error ? fetchError.message : 'Rezervasyon özeti oluşturulamadı.'); setState('error') } })
    return () => controller.abort()
  }, [request])

  const fallback = request?.fallback ?? '/'
  const visibleState = request ? state : 'error'
  ...
  return <main className="page-frame booking-summary-page" data-testid="booking-summary-page">
    ...
    {visibleState === 'ready' && summary && <>
      {summary.priceChanged && <FeedbackState tone="warning" title="Fiyat güncellendi" message={`Önceki ${summary.quotedTotal?.toLocaleString('tr-TR')} ${summary.currency} yerine güncel toplam ${summary.totalPrice.toLocaleString('tr-TR')} ${summary.currency}. Onay verirsen güncel fiyat geçerli olacak.`} />}

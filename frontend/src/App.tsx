...
function ProfilePage() {
  const { user } = useAuth()
  return <PageFrame eyebrow="HESABIM" title="Profil ve tercihlerin" description="Demo oturum bilgilerini ve uygulama tercihlerini bu alanda yönetin."><div className="profile-card"><span className="profile-avatar">{user?.name.slice(0, 1)}</span><div><h3>{user?.name}</h3><p>{user?.role === 'admin' ? 'Admin demo rolü' : 'Kayıtlı kullanıcı demo rolü'}</p></div></div></PageFrame>
}

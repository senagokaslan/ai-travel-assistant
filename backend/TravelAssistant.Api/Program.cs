...
app.MapGet("/api/hotels", async (string? q, DateOnly? checkIn, DateOnly? checkOut, int? adults, int? rooms, int? children, string? childAges, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    ...
    var roomCount = rooms ?? 1; var guestsPerRoom = (int)Math.Ceiling(((adults ?? 1) + (children ?? 0)) / (decimal)roomCount);
    await using var connection = new NpgsqlConnection(configuration.GetConnectionString("Postgres")); await connection.OpenAsync(cancellationToken);
    const string searchSql = "SELECT h.id, h.name, c.name, h.district, h.stars, h.rating, h.description, ARRAY(SELECT DISTINCT rn.name FROM hotel_rooms rn WHERE rn.hotel_id=h.id AND rn.is_active), ARRAY(SELECT DISTINCT feature FROM hotel_rooms rf CROSS JOIN LATERAL unnest(rf.features) feature WHERE rf.hotel_id=h.id AND rf.is_active), COALESCE((SELECT MIN(room_total) FROM (SELECT SUM(prr.nightly_price) AS room_total FROM hotel_rooms pr JOIN room_daily_rates prr ON prr.room_id=pr.id AND prr.stay_date>=@check_in AND prr.stay_date<@check_out WHERE pr.hotel_id=h.id AND pr.is_active AND pr.capacity>=@guests_per_room GROUP BY pr.id HAVING COUNT(*)=@nights AND MIN(prr.rooms_available)>=@rooms) priced), 0) * @rooms FROM hotels h JOIN travel_cities c ON c.id=h.city_id WHERE h.is_active AND (h.name ILIKE @like OR c.name ILIKE @like OR h.district ILIKE @like) AND EXISTS (SELECT 1 FROM hotel_rooms ar JOIN room_daily_rates arrr ON arrr.room_id=ar.id AND arrr.stay_date>=@check_in AND arrr.stay_date<@check_out WHERE ar.hotel_id=h.id AND ar.is_active AND ar.capacity>=@guests_per_room GROUP BY ar.id HAVING COUNT(*)=@nights AND MIN(arrr.rooms_available)>=@rooms) ORDER BY h.rating DESC";
    await using var command = new NpgsqlCommand(searchSql, connection); command.Parameters.AddWithValue("like", $"%{q!.Trim()}%"); command.Parameters.AddWithValue("check_in", arrival.ToDateTime(TimeOnly.MinValue)); command.Parameters.AddWithValue("check_out", departure.ToDateTime(TimeOnly.MinValue)); command.Parameters.AddWithValue("guests_per_room", guestsPerRoom); command.Parameters.AddWithValue("rooms", roomCount); command.Parameters.AddWithValue("nights", nights);
    await using var reader = await command.ExecuteReaderAsync(cancellationToken); var hotels = new List<object>(); while (await reader.ReadAsync(cancellationToken)) hotels.Add(new { id = reader.GetGuid(0), name = reader.GetString(1), city = reader.GetString(2), district = reader.GetString(3), stars = reader.GetInt16(4), rating = reader.GetDecimal(5), description = reader.GetString(6), rooms = reader.GetFieldValue<string[]>(7), features = reader.GetFieldValue<string[]>(8), totalPrice = reader.GetDecimal(9) }); return Results.Ok(hotels);
});

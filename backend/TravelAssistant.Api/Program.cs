...
app.MapGet("/api/chat/conversations", async (HttpRequest request, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    if (!TryGetSession(request, sessions, out var session)) return Results.Unauthorized();
    await using var connection = new NpgsqlConnection(configuration.GetConnectionString("Postgres"));
    await connection.OpenAsync(cancellationToken);
    await using var command = new NpgsqlCommand("SELECT id, title, created_at, updated_at FROM chat_conversations WHERE user_id=@user_id ORDER BY updated_at DESC", connection);
...
app.MapPost("/api/chat/conversations", async (HttpRequest request, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    if (!TryGetSession(request, sessions, out var session)) return Results.Unauthorized();
    await using var connection = new NpgsqlConnection(configuration.GetConnectionString("Postgres"));
    await connection.OpenAsync(cancellationToken);
    await using var command = new NpgsqlCommand("INSERT INTO chat_conversations (user_id) VALUES (@user_id) RETURNING id, title, created_at, updated_at", connection);
    command.Parameters.AddWithValue("user_id", session.Id);
    await using var reader = await command.ExecuteReaderAsync(cancellationToken);
...
app.MapGet("/api/chat/conversations/{conversationId:guid}/messages", async (Guid conversationId, HttpRequest request, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    if (!TryGetSession(request, sessions, out var session)) return Results.Unauthorized();
    await using var connection = new NpgsqlConnection(configuration.GetConnectionString("Postgres"));
    await connection.OpenAsync(cancellationToken);
...
app.MapPost("/api/chat/conversations/{conversationId:guid}/messages", async (Guid conversationId, ChatMessageRequest chatRequest, HttpRequest request, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    ...
    await using (var ownership = new NpgsqlCommand("SELECT EXISTS (SELECT 1 FROM chat_conversations WHERE id=@id AND user_id=@user_id)", connection))
    {
        ...
        ownership.Parameters.AddWithValue("user_id", session.Id);
    ...
    {
        insertUser.Parameters.AddWithValue("conversation_id", conversationId);
        insertUser.Parameters.AddWithValue("content", content);
        insertUser.Parameters.AddWithValue("client_id", chatRequest.ClientMessageId);
        var inserted = await insertUser.ExecuteScalarAsync(cancellationToken);
        if (inserted is null)
        {
            await using var duplicate = new NpgsqlCommand("SELECT id FROM chat_messages WHERE conversation_id=@conversation_id AND client_message_id=@client_id", connection);
            duplicate.Parameters.AddWithValue("conversation_id", conversationId);
            duplicate.Parameters.AddWithValue("client_id", chatRequest.ClientMessageId);
            userMessageId = (Guid)(await duplicate.ExecuteScalarAsync(cancellationToken))!;
            await using var existingReply = new NpgsqlCommand("SELECT id, content, metadata::text, created_at FROM chat_messages WHERE reply_to_message_id=@reply_to", connection);
            existingReply.Parameters.AddWithValue("reply_to", userMessageId);
    ...
    {
        insertAssistant.Parameters.AddWithValue("conversation_id", conversationId);
        insertAssistant.Parameters.AddWithValue("content", reply.Content);
        insertAssistant.Parameters.AddWithValue("reply_to", userMessageId);
        insertAssistant.Parameters.AddWithValue("metadata", reply.MetadataJson);
        await using var reader = await insertAssistant.ExecuteReaderAsync(cancellationToken);
        await reader.ReadAsync(cancellationToken);
        assistantId = reader.GetGuid(0); assistantCreatedAt = reader.GetDateTime(1);
    }
    ...
    {
        updateTitle.Parameters.AddWithValue("title", content);
        updateTitle.Parameters.AddWithValue("id", conversationId);
        await updateTitle.ExecuteNonQueryAsync(cancellationToken);
    }
...
record ChatMessageRequest(string? Content, Guid ClientMessageId);

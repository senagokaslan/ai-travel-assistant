...
app.MapGet("/api/flights", async (string? from, string? to, DateOnly? date, IConfiguration configuration, CancellationToken cancellationToken) =>
{
    ...
    await using var connection = new NpgsqlConnection(configuration.GetConnectionString("Postgres")); await connection.OpenAsync(cancellationToken);
    await using var command = new NpgsqlCommand("SELECT f.flight_number, al.name, al.iata_code, MIN(l.departure_at), MAX(l.arrival_at), COUNT(*) FROM flights f JOIN airlines al ON al.id=f.airline_id JOIN flight_legs l ON l.flight_id=f.id JOIN airports dep ON dep.id=l.departure_airport_id JOIN airports arr ON arr.id=l.arrival_airport_id WHERE f.status='scheduled' AND (@from='' OR dep.iata_code=@from) AND (@to='' OR arr.iata_code=@to) AND (@date IS NULL OR l.departure_at::date=@date) GROUP BY f.flight_number, al.name, al.iata_code ORDER BY MIN(l.departure_at)", connection); command.Parameters.AddWithValue("from", origin); command.Parameters.AddWithValue("to", destination); command.Parameters.AddWithValue("date", (object?)date?.ToDateTime(TimeOnly.MinValue) ?? DBNull.Value);
    await using var reader = await command.ExecuteReaderAsync(cancellationToken); var results = new List<object>(); while (await reader.ReadAsync(cancellationToken)) results.Add(new { flightNumber = reader.GetString(0), airline = reader.GetString(1), airlineCode = reader.GetString(2), departureAt = reader.GetDateTime(3), arrivalAt = reader.GetDateTime(4), stops = reader.GetInt64(5) - 1 }); return Results.Ok(results);
});

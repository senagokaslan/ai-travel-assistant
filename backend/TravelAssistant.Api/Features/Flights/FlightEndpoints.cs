...
internal static class FlightEndpoints
{
    ...
    private static async Task<IResult> SearchFlightsAsync(string? from, string? to, DateOnly? date, DateOnly? returnDate, string? tripType, int? adults, int? children, int? infants, int? passengers, IConfiguration configuration, CancellationToken cancellationToken)
    {

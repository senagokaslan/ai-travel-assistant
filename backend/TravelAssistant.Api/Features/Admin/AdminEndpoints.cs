...
internal static class AdminEndpoints
{
    ...
    {
        app.MapGet("/api/admin/summary", async (HttpRequest request, IConfiguration configuration, SessionStore sessions, CancellationToken cancellationToken) =>
        {
            var session = await sessions.GetValidUserAsync(request, configuration, cancellationToken);
            return session?.Role == "admin" ? Results.Ok(new { message = "Yönetim erişimi doğrulandı." }) : Results.StatusCode(StatusCodes.Status403Forbidden);
        });
    ...
    {
        var session = await sessions.GetValidUserAsync(httpRequest, configuration, cancellationToken);
        if (session?.Role != "admin") return Results.StatusCode(StatusCodes.Status403Forbidden);
